  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible &&
          "bg-white/80 border border-gray-200/20",
        className
      )}
    >
      {children}
    ...
    >
      {items.map((item, idx) => (
        <Link
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-neutral-600 hover:text-neutral-800 transition-colors"
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between bg-transparent px-4 py-2 lg:hidden rounded-xl",
        visible &&
          "bg-white/80 border border-gray-200/20",
        className
      )}
...
>(
  (
    { as: Tag = "button", children, className, variant = "primary", ...props },
    ref
  ) => {
    const baseStyles =
      "px-4 py-2 rounded-sm bg-white text-black text-sm font-bold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center";
