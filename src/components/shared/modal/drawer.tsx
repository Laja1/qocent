...
export const DrawerModal = NiceModal.create(() => {
  ...
  const renderObjectAsTable = (obj: any, prefix = "") => {
    if (!obj || typeof obj !== "object") {
      return [
        <tr key={prefix || "value"}>
          <td className="px-3 py-2 font-medium text-gray-700 border-b text-xs">
            Value
          </td>
          <td className="px-3 py-2 text-gray-900 border-b break-words text-xs">
            {String(obj)}
          </td>
        </tr>,
      ];
      ...
      if (value === null || value === undefined) {
        ...
        rows.push(
          <tr key={`${fullKey}-header`}>
            <td
              className="px-3 py-2 font-semibold text-gray-800 bg-gray-100 border-b text-xs"
              colSpan={2}
            >
        ...
        rows.push(
          <tr key={fullKey}>
            <td className="px-3 py-2 font-medium text-xs text-gray-700 border-b">
              {formatKey(fullKey)}
            </td>
            <td className="px-3 py-2 border-b text-xs">
              <span
            ...
            {details ? (
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <table className="w-full">
                  ...
                  <tbody>{renderObjectAsTable(details)}</tbody>
                </table>
              </div>
            ) : (
