...
import type {
  ConfigResponse,
  resourceType,
} from "@/models/response/resourceResponse";
...
export const UpdateResources = () => {
  ...
  const locationState = location.state as resourceType;
  console.log(locationState, "sss");
  // ALL HOOKS MUST BE CALLED BEFORE ANY EARLY RETURNS
  const { data: configData, isLoading: isConfigLoading } = useGetConfigQuery(
    {
      serviceId: locationState?.resourceContainerType || "",
  ...
  const { data: resourceTemplate, isLoading } = useGetResourceTemplateQuery(
    {
      resource: locationState?.resourceContainerType || "",
  ...
  return (
    <div className="flex flex-col">
      <Header
        title={`Update ${locationState.resourceContainerType}`}
        description="A server can have one or more server houses. A server house is provided by a provider."
      />
        ...
        <div className="bg-gradient-to-r flex justify-between from-black to-gray-800 rounded-t-md px-3 sm:px-5 py-5">
          <div>
            ...
            {/* <p className="text-xs text-gray-400 leading-tight">
              {locationState?.resourceCheckerId}
            </p> */}
          </div>
          <div>
